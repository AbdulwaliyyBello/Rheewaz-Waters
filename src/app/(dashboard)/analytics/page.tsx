...
import { C, Card, naira } from "@/lib/ui";
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    setData(null);
    api<{ data: Row[] }>(`/api/analytics?range=${range}`).then((res) => setData(res.data));
  }, [range]);
  ...
  return (
    <div>
        ...
        {["1W", "1M", "6M", "1Y"].map((r) => (
          <button
            ...
            style={{
              padding: "7px 14px",
              borderRadius: 999,
              border: `1px solid ${r === range ? C.teal : C.line}`,
              background: r === range ? C.teal : "#fff",
              color: r === range ? "#fff" : C.inkSoft,
              fontSize: 13,
              fontWeight: 600,
              cursor: "pointer",
            }}
      ...
      {!data ? (
        <div style={{ padding: 30, textAlign: "center", color: C.inkSoft }}>Loading analytics…</div>
      ) : data.length === 0 ? (
        <div style={{ padding: 30, textAlign: "center", color: C.inkSoft }}>No data yet for this period. Analytics fill in as weeks are recorded.</div>
      ) : (
        <>
          ...
          <Card style={{ padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 12, color: C.inkSoft, marginBottom: 12 }}>Dashed line includes nylon roll spend for true long-term profitability</div>
