...
import { C, Card, Stat, Field, Btn, naira, weekLabel, inputStyle, DAY_LABEL, WEEKDAYS } from "@/lib/ui";
...
export default function ExpensesPage() {
  ...
  const load = () => api<Data>("/api/expenses").then((res) => {
    setData(res);
    const d = res.days.find((x) => x.weekday === day);
    setFactory(d?.factory ?? 0);
  });
  useEffect(() => { load(); }, []); // eslint-disable-line

  useEffect(() => {
    const d = data?.days.find((x) => x.weekday === day);
    setFactory(d?.factory ?? 0);
    setNylonYes(false);
    setNylonAmount(0);
  }, [day, data]);
  ...
  const save = async () => {
    ...
    await api("/api/expenses", {
      method: "POST",
      body: JSON.stringify({ weekday: day, factoryAmount: factory, nylonYes, nylonAmount }),
    });
    await load();
    setSaving(false);
  };
    ...
    <div>
      ...
      <Field label="Day">
        <select style={inputStyle} value={day} onChange={(e) => setDay(e.target.value)}>
          {WEEKDAYS.map((wd) => {
            const d = data.days.find((x) => x.weekday === wd);
            return (
              <option key={wd} value={wd}>
                {DAY_LABEL[wd]} {d?.submitted ? "· recorded" : "· pending"}
              </option>
            );
          })}
        </select>
      ...
      <Card style={{ padding: 18, marginBottom: 16 }}>
        <Field label="Factory expense (₦)">
          <input type="number" min={0} style={inputStyle} value={factory} onChange={(e) => setFactory(Number(e.target.value))} />
        </Field>
        <Field label="Nylon rolls supplied?">
          <select style={inputStyle} value={nylonYes ? "yes" : "no"} onChange={(e) => setNylonYes(e.target.value === "yes")}>
            <option value="no">No</option>
            <option value="yes">Yes</option>
          </select>
        </Field>
        {nylonYes && (
          <Field label="Nylon roll amount (₦)">
            <input type="number" min={0} style={inputStyle} value={nylonAmount} onChange={(e) => setNylonAmount(Number(e.target.value))} />
          </Field>
        )}
        <Btn onClick={save} disabled={saving} style={{ width: "100%" }}>
          {saving ? "Saving…" : `Save ${DAY_LABEL[day]}'s expenses`}
        </Btn>
      </Card>

      <div style={{ fontSize: 11.5, color: C.inkSoft, marginBottom: 16 }}>
        Nylon roll spend is tracked separately and excluded from the weekly Factory Expenses total — it only appears in the Boss's long-term analytics.
      </div>
