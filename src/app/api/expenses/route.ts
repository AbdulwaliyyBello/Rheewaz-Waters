...
export async function GET() {
  ...
  const days = WEEKDAYS.map((wd) => {
    ...
    return { weekday: wd, factory: num(f?.amount), submitted: f?.submitted ?? false, nylon: nylonForDay.map((n) => ({ id: n.id, amount: num(n.amount) })) };
  });
  return NextResponse.json({ weekStart: weekStartISO, days, factoryTotal: factoryRows.reduce((a, r) => a + num(r.amount), 0), nylonTotal: nylonRows.reduce((a, r) => a + num(r.amount), 0) });
}
...
export async function POST(req: NextRequest) {
  ...
  await db.update(schema.factoryExpenses).set({ amount: String(factoryAmount), submitted: true, createdBy: session!.userId, updatedAt: new Date() })
  ...
  if (nylonYes && nylonAmount > 0) {
    await db.insert(schema.nylonRollExpenses).values({ weekId, weekday: weekday as any, date: new Date(dateISO), amount: String(nylonAmount), createdBy: session!.userId });
  }
